...
prerelease *below* its release (`v0.30.2-amadeus.1` < `v0.30.2`), which is why consumers
should pin the fork through a `replace` directive rather than relying on `@latest`:

```
require k8s.io/client-go v0.30.2
replace k8s.io/client-go => github.com/AmadeusITGroup/client-go v0.30.2-amadeus.1
```
...
This branch, `amadeus-release-0.30`, tracks the upstream v0.30.x line. The fork maintains
one branch per upstream minor release; see that branch's copy of this file for its own
entries.

## v0.30.2-amadeus.1

Upstream base: `k8s.io/client-go` v0.30.2 (commit `592d89167`, Go 1.22).
